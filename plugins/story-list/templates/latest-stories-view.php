		...
		<ul id="sorted_list" class="sort_list">
			<?php if ($posts_list):
				...
				foreach ($posts_list as $story): ?>
					<li id="<?php echo esc_attr($story->ID); ?>">
						<?php echo $i . '. ' . esc_html($story->post_title) . ' <span class="postedDate">' . date("g:i a j F Y", strtotime($story->post_date)) . '</span> '; ?>
		...
		<ul id="un_sorted_list" class="sort_list">
			<?php if ($posts_list):
				foreach ($posts_list as $story):
					...
					?>
					<li id="<?php echo esc_attr($story->ID); ?>">
						<?php echo esc_html($story->post_title) . ' <span class="postedDate">' . date("g:i a j F Y", strtotime($story->post_date)) . '</span> '; ?>
						<button class="st_delete">X</button>
					</li>
				<?php endforeach;
			endif; ?>
